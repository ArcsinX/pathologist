...
fn member_short_name(source: &str, node: Node) -> Option<String> {
    fn walk(source: &str, n: Node) -> Option<String> {
        ...
        // (`ERROR [operator ns::]`), or — with a second macro trailing the
        // member — the target as well
        // (`ERROR [operator int() const]`, `GUARDED_BY(m)` outside it). A
        // declarator *inside* the `ERROR` is therefore the target whenever
        // there is one, and only its absence means the target is still to
        // come; reading the siblings first would name the member after the
        // trailing macro, and reading everything inside would name it after
        // the target's own scope.
        if let Some(err) = conversion_keyword_error(n) {
            // A declarator *inside* the `ERROR` is the whole target.
            if let Some(target) = err
            ...
            // Otherwise the `ERROR` swallowed the keyword and whatever scope
            // the target carried (`ERROR [operator ns::]`), and the rest of
            // the target is the declarator standing beside it. Reading only
            // that declarator dropped the scope, so a macro-annotated
            ...
            let carried_scope = normalize_spacing(&source[keyword_end..err.end_byte()]);
            let rest = n
                .children(&mut n.walk())
                .filter(|c| c.start_byte() >= err.end_byte())
                .find_map(|c| walk(source, c))?;
